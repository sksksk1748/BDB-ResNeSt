    ...
    def save_incorrect_pairs(self, distmat, queryloader, galleryloader, 
        ...
        for i in range(m):
            ...
            for j in range(10):
                ...
                # add by tim 2022/03/22
                # ref: https://www.delftstack.com/zh-tw/howto/matplotlib/how-to-hide-axis-text-ticks-and-or-tick-labels-in-matplotlib/
                # ref: https://stackoverflow.com/questions/1982770/matplotlib-changing-the-color-of-an-axis
                # ref: https://stackoverflow.com/questions/44257965/matplotlib-how-to-change-de-color-and-width-of-axes
                # if query image ID equal to gallery image ID 
                if q_pids[i] == g_pids[gallery_index] : 
                   axes[j+1].axes.xaxis.set_visible(False)
                   axes[j+1].yaxis.set_visible(False)
                   axes[j+1].spines['bottom'].set_color('green')
                   axes[j+1].spines['top'].set_color('green')
                   axes[j+1].spines['right'].set_color('green')
                   axes[j+1].spines['left'].set_color('green')
                
                   axes[j+1].spines['bottom'].set_linewidth(3)
                   axes[j+1].spines['top'].set_linewidth(3)
                   axes[j+1].spines['right'].set_linewidth(3)
                   axes[j+1].spines['left'].set_linewidth(3)
                else:
                   axes[j+1].axes.xaxis.set_visible(False)
                   axes[j+1].yaxis.set_visible(False)
                   axes[j+1].spines['bottom'].set_color('red')
                   axes[j+1].spines['top'].set_color('red')
                   axes[j+1].spines['right'].set_color('red')
                   axes[j+1].spines['left'].set_color('red')

                   axes[j+1].spines['bottom'].set_linewidth(2)
                   axes[j+1].spines['top'].set_linewidth(2)
                   axes[j+1].spines['right'].set_linewidth(2)
                   axes[j+1].spines['left'].set_linewidth(2)                
                # add by tim 2022/03/22
            ...
            # add by tim 2022/03/22 for svg file
            #fig.savefig(os.path.join(savefig, '%d.svg' %q_pids[i])) # for svg file
