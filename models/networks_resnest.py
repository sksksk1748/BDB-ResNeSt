...
class BFE(nn.Module):
    def __init__(self, num_classes, width_ratio=0.5, height_ratio=0.5):
        ...
        # Layer 4 no downsample
        resnet = resnest50(pretrained=True, dilation=2)
        
        # Layer 4 have downsample
        ...
        self.res_part[0].load_state_dict(resnet.layer4.state_dict())
    ...
    def forward(self, x):
        ...
        # add by tim 2022/05/03
        # get no mask feature map (erased height ratio : 0)
        #self.feature_map_to_png(x, 1)

        x = self.batch_crop(x)

        # add by tim 2022/05/03
        # get mask feature map (erased height ratio : 0.1 ~ 1.0)
        #self.feature_map_to_png(x, 2)
    ...
    def get_model_name(self):
        return '================ Now load ResNest50 ================'

    # add by tim 2022/05/03
    # Save feature map 
    def feature_map_to_png(self, x, flag):
        print("Saving feature map")
        os.makedirs('./pytorch-ckpt/test_fig3_eval/savefig', exist_ok=True)
        fig, axes =plt.subplots(1, 1, figsize=(12, 8))
        axes.set_title("feature map")
        axes.set_axis_off()
        axes.imshow(x.data.cpu().numpy()[0,1,:,:])
        if(flag == 1):
            fig.savefig(os.path.join('./pytorch-ckpt/test_fig3_eval/savefig', 'feature_map.png'))
        elif(flag == 2):
            fig.savefig(os.path.join('./pytorch-ckpt/test_fig3_eval/savefig', 'feature_map_mask.png'))
        plt.close(fig)
