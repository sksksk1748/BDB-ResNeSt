...
class BFE(nn.Module):
    def __init__(self, num_classes, width_ratio=0.5, height_ratio=0.5):
        ...
        # add by tim 2021/10/13
        #self.global_softmax_1 = nn.Linear(1024, 512)
        #self.global_softmax = nn.Linear(512, num_classes)
        #self.global_softmax_1.apply(weights_init_kaiming)
        # add by tim 2021/10/13 
    ...
    def forward(self, x):
        ...
        # add by tim 2021/10/13
        #global_softmax_class = self.global_softmax_1(global_triplet_feature)
        #global_softmax_class = self.global_softmax(global_softmax_class)
        # add by tim 2021/10/13
        softmax_features.append(global_softmax_class)
        triplet_features.append(global_triplet_feature)
        # add by tim 2021/10/13
        #print("global_triplet_feature : " + str(global_triplet_feature.shape))
        # add by tim 2021/10/13
        ...
        if self.training:
            ...
            # add by tim 2021/10/13
            #print(torch.cat(predict, 1).shape)
            # add by tim 2021/10/13
