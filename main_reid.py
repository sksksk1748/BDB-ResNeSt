...
def train(**kwargs):
    ...
    if opt.model_name == 'softmax' or opt.model_name == 'softmax_triplet':
        ...
        if opt.datatype == "person":
            # 2021/03/27 Test summary
            # 作者最初的做法
            #model = BFE(dataset.num_train_pids, 1.0, 0.33)

            # 看模型架構
            #model = BFE(dataset.num_train_pids, 1.0, 0.33).cuda()
            #summary(model,(3,384,128))

            # 自己設定 erased_height_ratio
            model = BFE(dataset.num_train_pids, 1.0, opt.erased_height_ratio)
    ...
    #    optimizer = torch.optim.AdamW(optim_policy, lr=opt.lr, weight_decay=opt.weight_decay)

    #2021/04/02 modify by tim
    # get optimizer
    if opt.optim == "sgd":
        optimizer = torch.optim.SGD(optim_policy, lr=opt.lr, momentum=0.9, weight_decay=opt.weight_decay)
        print("================ Now load SGD ================")
    	...
    	optimizer = torch.optim.AdamW(optim_policy, lr=opt.lr, weight_decay=opt.weight_decay)
    	print("================ Now load AdamW ================")
    elif opt.optim == "radam":
    	optimizer = radam.RAdam(optim_policy, lr=opt.lr, weight_decay=opt.weight_decay)
    	print("================ Now load RAdam ================")
    else:
        optimizer = torch.optim.Adam(optim_policy, lr=opt.lr, weight_decay=opt.weight_decay)
        print("================ Now load Adam ================")
